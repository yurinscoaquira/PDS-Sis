    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Editar Tipo de Trámite</h1>
                    <p class="mt-2 text-gray-600">Modifica la información del tipo de trámite: <strong>{{ $tipoTramite->nombre }}</strong></p>
                </div>
        ...
        <div class="bg-white shadow-sm rounded-lg">
            <form method="POST" action="{{ route('tipos-tramite.update', $tipoTramite) }}" class="space-y-6 p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $tipoTramite->nombre) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="codigo" id="codigo" value="{{ old('codigo', $tipoTramite->codigo) }}"
                               class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="gerencia_id" id="gerencia_id" required
                            ...
                            @foreach($gerencias as $gerencia)
                                <option value="{{ $gerencia->id }}" {{ old('gerencia_id', $tipoTramite->gerencia_id) == $gerencia->id ? 'selected' : '' }}>
                                    {{ $gerencia->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            ...
                            <input type="number" name="costo" id="costo" step="0.01" min="0" value="{{ old('costo', $tipoTramite->costo) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="tiempo_estimado_dias" id="tiempo_estimado_dias" min="1" max="365" value="{{ old('tiempo_estimado_dias', $tipoTramite->tiempo_estimado_dias) }}"
                               class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                               required>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="descripcion" id="descripcion" rows="4"
                              class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                              placeholder="Descripción detallada del trámite">{{ old('descripcion', $tipoTramite->descripcion) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-2 max-h-40 overflow-y-auto border border-gray-300 rounded-md p-3">
                        @php
                            $documentosIds = old('documentos', $tipoTramite->documentos->pluck('id')->toArray());
                        @endphp
                        @foreach($tiposDocumento as $tipoDoc)
                            <div class="flex items-center">
                                <input type="checkbox" name="documentos[]" id="doc_{{ $tipoDoc->id }}" 
                                       value="{{ $tipoDoc->id }}"
                                       {{ in_array($tipoDoc->id, $documentosIds) ? 'checked' : '' }}
                ...
                <div class="space-y-4">
                    <div class="flex items-center">
                        <input type="checkbox" name="requiere_pago" id="requiere_pago" value="1"
                               {{ old('requiere_pago', $tipoTramite->requiere_pago) ? 'checked' : '' }}
                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
